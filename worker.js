...
// 部署完成后在网址后面加上这个，获取订阅器默认节点，

let mytoken= 'auto';//快速订阅访问入口, 留空则不启动快速订阅
let addresses = [ 
];   
const testport = ["2053", "2096","443", "2087", "8443", "2083"];  
const usa = [ 
  /*  "cf.090227.xyz", 
...
for (let i = 0; i < usa.length; i++) {
    for (let j = 0; j < testport.length; j++) {  
        addresses.push(`${usa[i]}:${testport[j]}`)#美国; 
    }
}
...
for (let i = 0; i < hk.length; i++) {
    for (let j = 0; j < testport.length; j++) {  
        addresses.push(`${hk[i]}:${testport[j]}`)#香港; 
    }
}
// 设置优选地址api接口
let addressesapi = [
	//'https://raw.githubusercontent.com/hk1996yhx/youxuanip/main/youxuanip.txt',  
];

let DLS = 5;//速度下限
let addressescsv = [
	'https://raw.githubusercontent.com/hk1996yhx/youxuanip/main/result.csv' //iptest测速结果文件。
];

let subconverter = "api.v1.mk"; //在线订阅转换后端，目前使用肥羊的订阅转换功能。支持自建psub 可自行搭建https://github.com/bulianglin/psub
let subconfig = "https://raw.githubusercontent.com/hk1996yhx/youxuanip/main/ACL4SSR.ini"; //订阅配置文件
		...
		try {
			...
			for (let i = 1; i < lines.length; i++) {
				...
				if (parseFloat(columns[speedIndex]) > DLS) {
					...
					const formattedAddress = `${ipAddress}:443`;
...
export default {
	async fetch (request, env) {
		...
		if (mytoken !== '' && url.pathname.includes(mytoken)) {
			host = env.HOST || "edtunnel.377020945.xyz";
			uuid = env.UUID || "c53ed257-580f-4080-8751-74aca113a437";
